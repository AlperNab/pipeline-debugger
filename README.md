...
**Domain:** `DevOps / CI/CD` · **Suite:** `Developer Productivity Suite` · **Accent:** `#60a5fa`
...
**Pipeline Debugger** is a standalone, browser-based AI workflow app for **DevOps / CI/CD**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Pipeline logs/config → failure diagnosis.
...
- config diff
- dependency/cache issue classifier
- retry safety
...
|---|---:|:---:|---|---|
| `pipeline_logs`<br>Pipeline logs | text | Yes | — | Affects input: Pipeline logs. |
| `config`<br>config | text | Yes | — | Affects input: config. |
...
| `repo`<br>repo | text | No | — | Affects customization: repo. |
| `branch`<br>branch | text | No | — | Affects customization: branch. |
| `environment`<br>environment | text | No | — | Affects customization: environment. |
| `severity`<br>severity | text | No | — | Affects customization: severity. |
| `retry_rules`<br>retry rules | textarea | No | — | Affects customization: retry rules. |
| `secret_redaction`<br>secret redaction | text | No | — | Affects customization: secret redaction. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9146
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9146`. |
...
- Export download
- Job history
- GitHub Actions/GitLab/Jenkins optional
...
```bash
curl http://127.0.0.1:9146/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9146/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9146 uvicorn app.main:app --host 0.0.0.0 --port 9146
```
...
- `plugins/pipeline-debugger.json` for plugin metadata.
